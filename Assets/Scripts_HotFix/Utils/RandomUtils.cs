...
public class RandomUtils
{
    ...
    private static System.Random Random
    {
        get
        {
            if (_random == null) { _random = new System.Random(DateTime.Now.Second); }
            return _random;
        }
    }
    ...
    /// <typeparam name="T">数组的类型</typeparam>
    /// <param name="arr">需要随机排序的数组</param>
    public static void GetRandomList<T>(List<T> arr)
    {
    ...
    #region
    public static Vector3 GetRandomPos(Vector3 center, Vector3 range)
    {
        float x = center.x + UnityEngine.Random.Range(-range.x, range.x);
        float y = center.y + UnityEngine.Random.Range(-range.y, range.y);
        float z = center.z + UnityEngine.Random.Range(-range.z, range.z);
