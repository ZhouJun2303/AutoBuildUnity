...
namespace BM
{
    public static partial class AssetComponent
    {
        ...
        public static async ETTask<UpdateBundleDataInfo> CheckAllBundlePackageUpdate(Dictionary<string, bool> bundlePackageNames)
        {
            ...
            foreach (var bundlePackageInfo in bundlePackageNames)
            {
                ...
                if (remoteVersionLog == null)
                {
                    ...
                    updateBundleDataInfo.AddErrorInfo($"Cannot Find Remote Bundle VersionFile:{bundlePackageName}");
                    return updateBundleDataInfo;
                }
                //创建各个分包对应的文件夹
                ...
                //获取本地的VersionLog
                string localVersionLogExistPath = BundleFileExistPath(bundlePackageName, ConstantDefine.VersionLogsFile, true);
        ...
        private static async ETTask CalcNeedUpdateBundle(UpdateBundleDataInfo updateBundleDataInfo, string bundlePackageName, string remoteVersionLog, string localVersionLog)
        {
            ...
            //判断StreamingAsset下资源
            string streamingAssetLogPath = Path.Combine(AssetComponentConfig.LocalBundlePath, bundlePackageName, ConstantDefine.VersionLogsFile);
        ...
        public static async ETTask DownLoadUpdate(UpdateBundleDataInfo updateBundleDataInfo)
        {
            ...
            foreach (string packageName in updateBundleDataInfo.PackageNeedUpdateBundlesInfos.Keys)
            {
                ...
                {
                    byte[] fileLogsData = await DownloadBundleHelper.DownloadDataByUrl(Path.Combine(AssetComponentConfig.BundleServerUrl, packageName, ConstantDefine.FileLogsFile));
                    byte[] dependLogsData = await DownloadBundleHelper.DownloadDataByUrl(Path.Combine(AssetComponentConfig.BundleServerUrl, packageName, ConstantDefine.DependLogsFile));
                    byte[] groupLogsData = await DownloadBundleHelper.DownloadDataByUrl(Path.Combine(AssetComponentConfig.BundleServerUrl, packageName, ConstantDefine.GroupLogsFile));
                    ...
                    CreateUpdateLogFile(Path.Combine(AssetComponentConfig.HotfixPath, packageName, ConstantDefine.FileLogsFile),
                        System.Text.Encoding.UTF8.GetString(fileLogsData));
                    CreateUpdateLogFile(Path.Combine(AssetComponentConfig.HotfixPath, packageName, ConstantDefine.DependLogsFile),
                        System.Text.Encoding.UTF8.GetString(dependLogsData));
                    CreateUpdateLogFile(Path.Combine(AssetComponentConfig.HotfixPath, packageName, ConstantDefine.GroupLogsFile),
                        System.Text.Encoding.UTF8.GetString(groupLogsData));
                }
                byte[] versionLogsData = await DownloadBundleHelper.DownloadDataByUrl(Path.Combine(AssetComponentConfig.BundleServerUrl, packageName, ConstantDefine.VersionLogsFile));
                ...
                CreateUpdateLogFile(Path.Combine(AssetComponentConfig.HotfixPath, packageName, ConstantDefine.VersionLogsFile),
