...
namespace BM
{
    public class LoadHandler : LoadHandlerBase
    {
        ...
        /// 预下载异步等待加载的Task
        /// </summary>
        internal List<ETTask> PreDownloadAwaitEtTasks = new List<ETTask>();
        ...
        /// 异步加载所有的Bundle
        /// </summary>
        internal async ETTask LoadAsync(bool isPreDownload = false, Action failCB = null)
        {
            ...
            int failCount = 0;
            void subFailCallback()
            {
                failCount++;
            }

            LoadAsyncLoader(_loadBase, tcs, isPreDownload, subFailCallback).Coroutine();
            for (int i = 0; i < _loadDepends.Count; i++)
            {
                LoadAsyncLoader(_loadDepends[i], tcs, isPreDownload, subFailCallback).Coroutine();
            }
            for (int i = 0; i < _loadDependFiles.Count; i++)
            {
                LoadAsyncLoader(_loadDependFiles[i], tcs, isPreDownload, subFailCallback).Coroutine();
            }
            for (int i = 0; i < _loadDependGroups.Count; i++)
            {
                LoadAsyncLoader(_loadDependGroups[i], tcs, isPreDownload, subFailCallback).Coroutine();
            }
            await tcs;
            if (failCount > 0 && failCB != null)
            {
                failCB();
                LoadState = LoadState.NoLoad;
            }
