...
namespace BM
{
    public static partial class AssetComponent
    {
        ...
        /// 获取分包的更新索引列表
        /// </summary>
        public static async ETTask<string> GetRemoteBundlePackageVersionLog(string bundlePackageName)
        {
            byte[] data = await DownloadBundleHelper.DownloadDataByUrl(Path.Combine(AssetComponentConfig.BundleServerUrl, bundlePackageName, ConstantDefine.VersionLogsFile), 6);
            if (data == null)
            {
    ...
    public class UpdateBundleDataInfo
    {
        ...
        /// 错误日志
        /// </summary>
        public List<string> ErrorInfos { get; private set; } = new List<string>();
