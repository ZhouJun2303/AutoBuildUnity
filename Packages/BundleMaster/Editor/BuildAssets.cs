...
namespace BM
{
    public class BuildAssets : EditorWindow
    {
        ...
        public static void BuildAllBundleMenuItem()
        {
            if (!EditorUtility.DisplayDialog("二次确认", "是否确认执行“构建AssetBundle”操作？", "确认"))
            {
                Debug.Log("取消了操作！");
                return;
            }
        ...
        public static void BuildAllBundle()
        {
            ...
            // 检查每个AssetsSetting配置的文件路径是否存在
            foreach (AssetsSetting assetsSetting in assetsSettings)
            {
                if (assetsSetting is AssetsLoadSetting tempLoadSetting)
                {
                    tempLoadSetting.CheckResPathIsExist();
                }
                else if (assetsSetting is AssetsOriginSetting tempOriginSetting)
                {
                    tempOriginSetting.CheckResPathIsExist();
                }
                else
                {
                    throw new System.Exception("未处理的AssetingSetting子类：" + assetsSetting.GetType().FullName);
                }
            }
        ...
        [MenuItem("Tools/BuildAsset/删除build文件")]
        public static void CleanBuildTempFolder()
        {
            AssetLoadTable assetLoadTable = AssetDatabase.LoadAssetAtPath<AssetLoadTable>(BundleMasterWindow.AssetLoadTablePath);
            DeleteHelper.DeleteDir(assetLoadTable.BuildBundlePath);

            string encryptAssetFolderPath = Path.Combine(assetLoadTable.BuildBundlePath + "/../", assetLoadTable.EncryptPathFolder);
            DeleteHelper.DeleteDir(encryptAssetFolderPath);

            string streamingAssetPath = Path.Combine(Application.dataPath, "StreamingAssets");
            DeleteHelper.DeleteDir(streamingAssetPath);
        ...
        public static void CopyToStreamingAssets()
        {
            ...
            if (!EditorUtility.DisplayDialog("二次确认", "是否确认执行“Copy资源到StreamingAssets”操作？", "确认"))
            {
                Debug.Log("取消了操作！");
                return;
            }
            CopyFileToTargetFolder(Application.streamingAssetsPath);
        }

        [MenuItem("Tools/BuildAsset/Copy资源到本地服务器目录")]
        public static void CopyToLocalServer()
        {
            if (!EditorUtility.DisplayDialog("二次确认", "是否确认执行“Copy资源到本地服务器目录”操作？", "确认"))
            {
                Debug.Log("取消了操作！");
                return;
            }
            CopyFileToTargetFolder(AssetComponentConfig.LocalServerFolder);
        }

        public static void CopyFileToTargetFolder(string targetPath)
        {
            if (!Directory.Exists(targetPath))
            {
                Directory.CreateDirectory(targetPath);
            }
            DeleteHelper.DeleteDir(targetPath);
            AssetLoadTable assetLoadTable = AssetDatabase.LoadAssetAtPath<AssetLoadTable>(BundleMasterWindow.AssetLoadTablePath);
            foreach (AssetsSetting assetsSetting in assetLoadTable.AssetsSettings)
            {
                ...
                string directoryPath = Path.Combine(targetPath, assetsLoadSetting.BuildName);
            ...
            foreach (AssetsSetting assetsSetting in assetLoadTable.AssetsSettings)
            {
                ...
                string directoryPath = Path.Combine(targetPath, assetsOriginSetting.BuildName);
                if (!Directory.Exists(directoryPath))
                {
                    Directory.CreateDirectory(directoryPath);
                }
            ...
            AssetDatabase.Refresh();
            AssetLogHelper.Log($"已将资源复制到{targetPath}");
        }
        ...
        private static void Build(AssetLoadTable assetLoadTable, AssetsLoadSetting assetsLoadSetting, HashSet<string> assetLoadPath, HashSet<string> alwaysIncludedShaders)
        {
            ...
            for (int i = 0; i < sceneAssets.Length; i++)
            {
                ...
                {
                    AssetLogHelper.LogError($"分包{assetsLoadSetting.BuildName} 的找不到第{i}个Scene,场景资源可能已删除！");
                }
                else
                {
            ...
            foreach (string file in files)
            {
                ...
                for (int i = 0; i < depends.Length; i++)
                {
                    ...
                    //脚本不能作为依赖
                    if (depend.EndsWith(".cs") || depend.EndsWith(".dll"))
                    {
                        continue;
                    }
                    //shader单独进包
            ...
            if (manifest == null)
            {
                AssetLogHelper.LogError("构建ab包失败: " + assetsLoadSetting.BuildName);
                return;
            }
            //保存未加密的版本号文件
            ...
            if (assetsLoadSetting.EncryptAssets)
            {
                ...
                //复制Log信息
                File.Copy(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, ConstantDefine.FileLogsFile), Path.Combine(encryptAssetPath, ConstantDefine.FileLogsFile));
                File.Copy(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, ConstantDefine.DependLogsFile), Path.Combine(encryptAssetPath, ConstantDefine.DependLogsFile));
                File.Copy(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, ConstantDefine.GroupLogsFile), Path.Combine(encryptAssetPath, ConstantDefine.GroupLogsFile));
            }
            //存储记录所有资源的加载路径
        ...
        private static void SaveLoadLog(AssetLoadTable assetLoadTable, AssetsLoadSetting assetsLoadSetting, Dictionary<string, LoadFile> loadFiles, Dictionary<string, LoadDepend> loadDepends, Dictionary<string, LoadGroup> loadGroups)
        {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, ConstantDefine.FileLogsFile)))
            {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, ConstantDefine.DependLogsFile)))
            {
                StringBuilder sb = new StringBuilder();
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(assetLoadTable.BuildBundlePath, assetsLoadSetting.BuildName, ConstantDefine.GroupLogsFile)))
            {
                StringBuilder sb = new StringBuilder();
        ...
        private static void SaveBundleVersionFile(string bundlePackagePath, AssetBundleManifest manifest, AssetsLoadSetting assetsLoadSetting, bool encrypt)
        {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(bundlePackagePath, ConstantDefine.VersionLogsFile)))
            {
                StringBuilder sb = new StringBuilder();
                ...
                foreach (string assetBundle in assetBundles)
                {
                    ...
                    {
                        Debug.LogError("some assetBundle name is null in: " + bundlePackagePath);
                        continue;
                    }
